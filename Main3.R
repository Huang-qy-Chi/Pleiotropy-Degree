...
dat = Gen_data(n = 1000, beta = c(0,0,0,0), Sigma0 = S1)
y = dat$Y
x = dat$X
alpha = 0.05
DEDGP(y,x,0.05)
...
re = rep(0,1000)
for(b in 1:1000){
  dat = Gen_data(n = 500, beta = c(1,1,0,0,rep(0,6)), Sigma0 = S1)
  y = dat$Y
  x = dat$X
  #DEDGP(y,x,0.05)
  #DGP(y,x,2,0.05)
  
  re[b] = DGP(y,x,2,0.05)$index
  cat("Loop:",b,"times.\r")
}
mean(re)
...
RR <- foreach(i = 1:1000,.combine = c,.packages = "LaplacesDemon")%dopar%{
  dat = Gen_data(n = 500, beta = c(1,0,0,0,rep(0,6)), Sigma0 = S3)
  y = dat$Y
  x = dat$X
  DGP(y,x,K=2,alpha = 0.05)$index
}
stopImplicitCluster()
stopCluster(cl)
mean(RR)
...
registerDoParallel(cl)
RR <- foreach(i = 1:1000,.combine = c,.packages = "LaplacesDemon")%dopar%{
  dat = Gen_data(n = 500, beta = c(1,0,0,0), Sigma0 = S3)
  y = dat$Y
  x = dat$X
  DGP(y,x,K=2,alpha = 0.01)$index
}
stopImplicitCluster()
stopCluster(cl)
mean(RR)
...
for(p in 1:2){ #alpha
  for(r in 1:3){ #COrr
    ...
    for(m in 1:2){ #sample size
      for(u in 1:4){ #different hypothesis
        RR <- foreach(i=1:1000,.combine = c,.packages = "LaplacesDemon")%dopar%{
          ...
          y = result$Y
          x = result$X
          DGP(y,x,K=2,alpha = alpha[p])$index
        }
        RS[m,u]=mean(RR)
      }
    }
    ...
    write.table(RS,sep=",",file =paste("C:/Users/Administrator/Desktop/Pleio Degree/DGP-4-",100*alpha[p],"/DGP",10*corr[r],".csv",sep=""),row.names = T)
    beep(1)
    Sys.sleep(20)
  }
}
...
for(p in 1:2){ #alpha
  for(r in 1:3){ #COrr
    ...
    rownames(RS) = c("n=500","n=1000")
    colnames(RS) = c("K^*=0","K^*=1","K^*=2","K^*=3")  
    write.table(RS,sep=",",file =paste("C:/Users/Administrator/Desktop/Pleio Degree/DGP-10-",100*alpha[p],"/DGP10_",10*corr[r],".csv",sep=""),row.names = T)
    beep(1)
    Sys.sleep(20)
  }
}
